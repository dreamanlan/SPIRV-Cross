	...
	for (const auto &spv_func : spv_function_implementations)
	{
		...
		case SPVFuncImplGatherConstOffsets:
			...
			for (uint32_t i = 0; i < texture_addr_space_count; i++)
			{
				...
				statement("case component::x:");
				statement("    rslts[i] = t.gather(s, params..., int2(coffsets[i]), component::x);");
				statement("    break;");
				statement("case component::y:");
				statement("    rslts[i] = t.gather(s, params..., int2(coffsets[i]), component::y);");
				statement("    break;");
				statement("case component::z:");
				statement("    rslts[i] = t.gather(s, params..., int2(coffsets[i]), component::z);");
				statement("    break;");
				statement("case component::w:");
				statement("    rslts[i] = t.gather(s, params..., int2(coffsets[i]), component::w);");
				statement("    break;");
				end_scope();
				end_scope();
		...
		case SPVFuncImplGatherCompareConstOffsets:
			...
			for (uint32_t i = 0; i < texture_addr_space_count; i++)
			{
				...
				statement("for (uint i = 0; i < 4; i++)");
				begin_scope();
				statement("    rslts[i] = t.gather_compare(s, params..., int2(coffsets[i]));");
				end_scope();
				// Pull all values from the i0j0 component of each gather footprint
	...
	for (auto &r : resources)
	{
		...
		if (r.discrete_descriptor_alias)
		{
			if (r.var == r.discrete_descriptor_alias)
			{
				...
				if (!ep_args.empty())
					ep_args += ", ";
				ep_args += get_argument_address_space(var) + " const void* " + primary_name;
